...
public final class QREncoder {
     ...
     */
    public static QRCode encode(String content, ErrorCorrectionLevel ecLevel) throws IllegalArgumentException {
        return encode(content, ecLevel, null);
    }

    public static QRCode encode(String content,
            ErrorCorrectionLevel ecLevel,
            Map<?, ?> hints) throws IllegalArgumentException {
        ...
        if (mode == Mode.BYTE && (/*hasEncodingHint*/false || !DEFAULT_BYTE_MODE_ENCODING.equals(encoding))) {
     ...
     * provided data.
     *
     * @throws IllegalArgumentException if the data cannot fit in any version
     */
    private static Version recommendVersion(ErrorCorrectionLevel ecLevel,
            Mode mode,
            BitArray headerBits,
            BitArray dataBits) throws IllegalArgumentException {
    ...
    private static int chooseMaskPattern(BitArray bits,
            ErrorCorrectionLevel ecLevel,
            Version version,
            ByteMatrix matrix) throws IllegalArgumentException {
    ...
    private static Version chooseVersion(int numInputBits, ErrorCorrectionLevel ecLevel) throws IllegalArgumentException {
     ...
     */
    static void terminateBits(int numDataBytes, BitArray bits) throws IllegalArgumentException {
        int capacity = numDataBytes * 8;
        if (bits.getSize() > capacity) {
            throw new IllegalArgumentException("data bits cannot fit in the QR Code" + bits.getSize() + " > "
                    + capacity);
        }
        ...
        if (bits.getSize() != capacity) {
            throw new IllegalArgumentException("Bits size does not equal capacity");
        }
    }
    ...
    static void getNumDataBytesAndNumECBytesForBlockID(int numTotalBytes,
            ...
            int[] numECBytesInBlock) throws IllegalArgumentException {
        ...
        if (numEcBytesInGroup1 != numEcBytesInGroup2) {
            throw new IllegalArgumentException("EC bytes mismatch");
        }
        // 5 = 4 + 1.
        if (numRSBlocks != numRsBlocksInGroup1 + numRsBlocksInGroup2) {
            throw new IllegalArgumentException("RS blocks mismatch");
        }
        // 196 = (13 + 26) * 4 + (14 + 26) * 1
        if (numTotalBytes
                ...
                * numRsBlocksInGroup2)) {
            throw new IllegalArgumentException("Total bytes mismatch");
        }
    ...
    static BitArray interleaveWithECBytes(BitArray bits,
            int numTotalBytes,
            int numDataBytes,
            int numRSBlocks) throws IllegalArgumentException {
        ...
        if (numDataBytes != dataBytesOffset) {
            throw new IllegalArgumentException("Data bytes does not match offset");
        }
        ...
        if (numTotalBytes != result.getSizeInBytes()) {  // Should be same.
            throw new IllegalArgumentException("Interleaving error: " + numTotalBytes + " and "
     ...
     */
    static void appendLengthInfo(int numLetters, Version version, Mode mode, BitArray bits) throws IllegalArgumentException {
        int numBits = mode.getCharacterCountBits(version);
        if (numLetters >= (1 << numBits)) {
            throw new IllegalArgumentException(numLetters + " is bigger than " + ((1 << numBits) - 1));
        }
    ...
    static void appendBytes(String content,
            Mode mode,
            BitArray bits,
            String encoding) throws IllegalArgumentException {
        switch (mode) {
            case NUMERIC:
    ...
    static void appendAlphanumericBytes(String content, BitArray bits) throws IllegalArgumentException {
        int length = content.length();
        int i = 0;
        while (i < length) {
    ...
    static void append8BitBytes(String content, BitArray bits, String encoding)
        ...
        try {
            ...
            throw new IllegalArgumentException(uee.toString());
        }
        for (byte b : bytes) {
            bits.appendBits(b, 8);
        }
