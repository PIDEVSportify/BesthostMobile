...
import com.google.zxing.qrcode.Version.ECBlocks;
...
public final class Version {
  ...
  public static Version getProvisionalVersionForDimension(int dimension) {
    if (dimension % 4 != 1) {
    	throw new IllegalArgumentException("Dimension for square not divisible by 4");
    }
    try {
  ...
  static Version decodeVersionInformation(int versionBits) {
    ...
    for (int i = 0; i < VERSION_DECODE_INFO.length; i++) {
      ...
      // we have seen so far
      int bitsDifference = numBits(versionBits ^ targetVersion);
  ...
  /**
   * As Integer.bitCount(int) not implemented...
   * @param i
   * @return
   */
  private static int numBits(int i) {
	  int b = 0;
	  while (i > 0) {
		  if ((i & 0b01) > 0) {
			  b++;
