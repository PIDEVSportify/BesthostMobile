...
public class ProfileGui extends Form {
    ...
    public ProfileGui ()
    {
        ...
        ImageViewer profile_img = new ImageViewer(EncodedImage.createFromImage(Image.createImage(420,320, 0xffff0000),false));
        if (UserServices.user.getAvatar()!=null)
        {
            EncodedImage placeholder = EncodedImage.createFromImage(Image.createImage(400,280, 0xffff0000),false);
        ...
        //else    profile_img.setImage(null);

        Container cont_generalSettings = new Container(BoxLayout.y());
        TextField txt_first_name= new TextField(UserServices.user.getFirst_name());
        TextField txt_last_name= new TextField(UserServices.user.getLast_name());
//        TextField txt_cin = new TextField(user.getCin());
//        txt_cin.setEditable(false);
        ...
        btn_generalSettings.addActionListener(e->
        {
            ...
            UserServices.updateGeneralSettings(UserServices.user.getEmail(),txt_first_name.getText(),txt_last_name.getText());
            new ProfileGui().show();
        ...
        btn_securitySettings.addActionListener(e->
        {
            ...
            if (valid)
            {
                UserServices.updateSecuritySettings(UserServices.user.getEmail(),txt_password.getText());
...
//        acc_profile.getAllStyles().setFgColor(ColorUtil.rgb(200,15,60),true);
            ...
            Display.getInstance().openGallery( evt  -> {
                if(evt!=null && evt.getSource()!=null)
                {
                    ...
                    try {
                        ...
                        {
                            UserServices.uploadAvatar(filepath,UserServices.user.getEmail());
                            new ProfileGui().show();
                        ...
                        else
                        {
                            if (UserServices.user.getAvatar()!=null)
                            {
                                img1 = Image.createImage(FileSystemStorage.getInstance().openInputStream(UserServices.user.getAvatar())).scaled(400,280);
                                profile_img.setImage(img1);
                            }
                            else
                                profile_img.setImage(null);
        ...
        btn_take_photo.addActionListener(e->
        {
            ...
            if(photo!=null)
            {
                ...
                if(Dialog.show("Confirmer","","Confirmer","Annuler"))
                {
                    UserServices.uploadAvatar(photo,UserServices.user.getEmail());
                    new ProfileGui().show();
                ...
                else
                {
                    try {
                        if (UserServices.user.getAvatar()!=null)
                        {
                            img2 = Image.createImage(FileSystemStorage.getInstance().openInputStream(UserServices.user.getAvatar())).scaled(400,280);
                            profile_img.setImage(img2);
                        }
                        else
                            profile_img.setImage(null);
        ...
        btn_delete_account.addActionListener(e->
        {
            ...
            {
                UserServices.deleteUser(UserServices.user.getEmail());
                this.removeAll();
                new LoginGui().show();
            }
        });
